...
from mesonbuild.utils.pkgconfig import Repository as PkgConfigRepository
from mesonbuild.utils.pkgconfig import PkgConfigException
...
class InternalTests(BasePlatformTests):
    ...
    def _create_pkgconfig_repo(self, testdir: str, sysroot_dir: T.Optional[str] = None) -> PkgConfigRepository:
        return PkgConfigRepository([Path(testdir)],
    ...
    def test_pkgconfig_parser(self) -> None:
        ...
        self.assertEqual(pkg.get_name(), 'Test1')
        self.assertEqual(pkg.get_description(), 'This is test 1')
    ...
    def test_pkgconfig_parser_var_override(self) -> None:
        ...
        repo = self._create_pkgconfig_repo(testdir)
        pkg = repo.lookup('test1')
        self.assertIsNone(pkg.get_variable('notfound'))
        self.assertEqual(pkg.get_variable('notfound', define_variable=('notfound', 'def')), 'def')
        self.assertEqual(pkg.get_variable('tool_path', define_variable=('prefix', '/opt')), '/opt/bin/tool')

    def test_pkgconfig_parser_conflicts(self) -> None:
        testdir = os.path.join(self.unit_test_dir, '98 pkgconfig parser')
        repo = self._create_pkgconfig_repo(testdir)
        with self.assertRaises(PkgConfigException) as cm:
