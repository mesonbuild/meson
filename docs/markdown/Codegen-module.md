...
This function wraps flex, lex, reflex (but not RE/flex), and win_flex (on Windows). When using win_flex it will automatically add the `--wincompat` argument. When this function is run the first time, if no lex implementation has been found it will search for lex, flex, reflex, and win_flex, with any version. For greater control over what implementation is used, refer to the [find_lex method](#find_lex).
...
```meson
p1 = codegen.yacc('parser.y', header : '@BASENAME@.h', locations : 'locations.h')
headers = [p1[1], p1[2]]  # [header, locations]

p2 = codegen.yacc('parser.yy', locations : 'locations.hpp')
locations = p2[1]
```
