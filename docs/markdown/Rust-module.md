...
It has several advantages over invoking a `custom_target` directly:
  ...
  - Automatically passes some useful options for use in Meson, such as `--quite`
    and `--cpp-compat`
...
It takes the following positional arguments:
  ...
  - `infile`: the root rust file to bind, or a [`structured_sources`](Reference-manual_structured_sources.md) instance
