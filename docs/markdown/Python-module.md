...
`.dist-info/` metadata directory, for use with [[install_data]] or
[[custom_target]] when staging files defined by Python packaging PEPs
(e.g. PEP 770 SBOMs, PEP 639 license files). The returned value is a
placeholder that Python wheel build backends such as
[`meson-python`](https://mesonbuild.com/meson-python/) recognise and
route into `<distname>-<version>.dist-info/<subdir>/` inside the wheel.
When invoked via `meson install` directly, the placeholder expands to a
PEP 491-compatible filesystem layout under `{purelib}`.
