...
within the build directory, `config-subdir` must not exist in the
source directory and it must not contain '..' to refer to the parent
of the build directory. `build_dir` does not affect the install
directory path at all; `config.h` will be installed as
