...
Build a Linux kernel module. A `Kbuild` or `Makefile` must be found in the
current source directory. By default it builds for the current kernel release,
but the option `-Dexternal_project.kernel_build_dir=/path/x.y/build` can be used
to use a different kernel tree.

Keyword arguments:

- `verbose`: If set to `true` the output of sub-commands ran to configure, build
  and install the project will be printed onto Meson's stdout.
- `depends` : Specifies that this project depends on the specified
  target(s) that must be built before running `make` to build this external
  project.
- `build_by_default` : Causes, when set to true, to have this
  target be built by default even when no other targets depends on it.

Returns an opac object
