...
be executed by meson. If the current script was called by `subdir` the
execution returns to the calling dir and continues as if the script had
reached the end. If the current script is the top level script meson
...
return_to_calling_dir()
executable('main2', 'main2.cpp')
```

The executable `main` will be build, while the executable `main2` is not
build.
