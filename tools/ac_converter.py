...
help_message = """Usage: {0} <config.h.meson>
...
for tok in functions:
    if len(tok) == 3:
        ...
        print("  ['{0}', '{1}', '#include<{3}>'],".format(tokstr, fdata0, fdata1))
    else:
...
for elem, typename in sizes:
    print("cdata.set('{0}', cc.sizeof('{1}'))".format(elem, typename))
