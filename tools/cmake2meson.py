            ...
            for (tid, reg) in self.token_specification:
                ...
                if mo:
                    ...
                    if tid == 'comment':
                        ...
                        raise ValueError('lex: unknown element {0}'.format(tid))
                    break
            if not matched:
                raise ValueError('Lexer got confused line {0} column {1}'.format(lineno, col))
...
class Parser:
    ...
    def expect(self, s: str) -> bool:
        if self.accept(s):
            return True
        raise ValueError('Expecting {0} got {1}.'.format(s, self.current.tid), self.current.lineno, self.current.colno)
...
class Converter:
    ...
    def convert_args(self, args: typing.List[Token], as_array: bool = True) -> str:
        ...
        for i in args:
            if i.tid == 'id':
                res.append("'{0}'".format(i.value))
            elif i.tid == 'varexp':
                res.append('{0}'.format(i.value.lower()))
            elif i.tid == 'string':
                res.append("'{0}'".format(i.value))
            else:
                raise ValueError('Unknown arg type {0}'.format(i.tid))
        if len(res) > 1:
    ...
    def write_entry(self, outfile: typing.TextIO, t: Statement):
        ...
        if t.name == '_':
            ...
            mods = ["dependency('{0}')".format(i.value for i in t.args[1:])]
            if len(mods) == 1:
                line = '{0} = {1}'.format(varname, mods[0])
            else:
                line = '{0} = [{1}]'.format(varname, ', '.join(["'{0}'".format(i for i in mods)]))
        elif t.name == 'find_package':
            line = "{0}_dep = dependency('{1}')".format(t.args[0].value, t.args[0].value)
        elif t.name == 'find_library':
            line = "{0} = find_library('{1}')".format(t.args[0].value.lower(), t.args[0].value)
        elif t.name == 'add_executable':
            line = '{0}_exe = executable({1})'.format(t.args[0].value, self.convert_args(t.args, False))
            ...
            if t.args[1].value == 'SHARED':
                ...
                libcmd = 'library'
                args = t.args
            line = '{0}_lib = {1}({2})'.format(t.args[0].value, libcmd, self.convert_args(args, False))
        elif t.name == 'add_test':
            line = 'test({0})'.format(self.convert_args(t.args, False))
        elif t.name == 'option':
            ...
            postincrement = 1
            try:
                line = 'if {0}'.format(self.convert_args(t.args, False))
            ...
            preincrement = -1
            postincrement = 1
            line = 'elif {0}'.format(self.convert_args(t.args, False))
        elif t.name == 'else':
            preincrement = -1
            postincrement = 1
            line = 'else'
            ...
            line = '''# {0}({1})'''.format(t.name, self.convert_args(t.args))
            ...
            for o in self.options:
                ...
                if default is None:
                    ...
                    if default == 'OFF':
                        ...
                        typestr = ' type : \'string\','
                    defaultstr = ' value : {0},'.format(default)
                line = "option({0},{1}{2} description : '{3}')\n".format(optname,
                                                                         typestr,
                                                                         defaultstr,
                                                                         description)
                optfile.write(line)
