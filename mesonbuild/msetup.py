...
from .cross import CrossConfigRegistry, CrossConfigNotFoundError, DependencyChecker
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--cross-os',
                        default=None,
                        help='Cross-compile target OS (e.g. linux, windows, darwin). '
                             'Used together with --cross-arch to resolve a built-in '
                             'cross-compilation config.')
    parser.add_argument('--cross-arch',
                        default=None,
                        help='Cross-compile target CPU architecture (e.g. armhf, aarch64, x86_64). '
                             'Used together with --cross-os to resolve a built-in '
                             'cross-compilation config.')
...
def _resolve_cross_config(options: CMDOptions) -> None:
    """Resolve --cross-os and --cross-arch to a cross file path.

    Looks up the matching built-in (or user-override) cross-compilation
    configuration, checks that required dependencies are installed, and
    appends the resolved config path to ``options.cross_file`` so the
    standard cross-compilation pipeline can proceed unchanged.
    """
    if options.cross_os is None or options.cross_arch is None:
        raise MesonException(
            'Both --cross-os and --cross-arch must be specified together.\n'
            'Example: meson setup --cross-os=linux --cross-arch=armhf builddir')

    registry = CrossConfigRegistry()
    try:
        config_path = registry.resolve(options.cross_os, options.cross_arch)
    except CrossConfigNotFoundError as e:
        raise MesonException(str(e))

    mlog.log('Using cross-compilation config:', mlog.bold(str(config_path)))

    sections = machinefile.parse_machine_files([str(config_path)], os.getcwd())

    checker = DependencyChecker(sections)
    result = checker.check()
    if not result.all_ok:
        mlog.log('')
        mlog.log(mlog.red('ERROR:'), 'Missing cross-compilation dependencies:')
        mlog.log(DependencyChecker.format_report(result))
        mlog.log('')
        mlog.log('Please install the missing dependencies and re-run meson setup.')
        raise MesonException('Cross-compilation dependency check failed.')

    mlog.log('All cross-compilation dependencies satisfied.')

    if options.cross_file:
        mlog.warning(
            'Both --cross-file and --cross-os/--cross-arch specified. '
            'The resolved config will be appended to the cross file list.',
            fatal=False)

    options.cross_file.append(str(config_path))
...
def run(options: T.Union[CMDOptions, T.List[str]]) -> int:
    ...
    if options.cross_os is not None or options.cross_arch is not None:
        _resolve_cross_config(options)
