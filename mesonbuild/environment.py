...
class Environment:
    ...
    def create_pkgconfig_repo(for_machine: MachineChoice = MachineChoice.BUILD,
        ...
        # FIXME: Take default from CLI:
        # pkg-config pkg-config --variable pc_path
        # pkg-config pkg-config --variable pc_system_libdirs
        # pkg-config pkg-config --variable pc_system_includedirs
        default_system_paths: T.List[Path] = []
        default_library_paths: T.List[Path] = []
        default_include_paths: T.List[Path] = []
        if not is_cross or for_machine is MachineChoice.BUILD:
            # Get some default paths for build machine
        ...
        system_library_paths += _paths_from_env('LIBRARY_PATH') or []
