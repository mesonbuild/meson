...
class HermeticInterpreter(Interpreter):
    ...
    def __init__(self, build_info: Build,
        ...
        super().__init__(
            build_info,
            backend=NoneBackend(build_info),
    ...
    def func_dependency(self, node: mparser.BaseNode, args: T.Sequence[object],
        ...
        if dep_info:
            ...
            def get_variable(*, cmake: T.Optional[str] = None, pkgconfig: T.Optional[str] = None,
                             ...
                             pkgconfig_define: T.Optional[T.Tuple[T.Tuple[str, str], ...]] = None) -> str:  # fmt: skip
                if configtool and configtool in configtool_checks:
                    return configtool_checks[configtool]
                if pkgconfig and pkgconfig in pkgconfig_checks:
                    return pkgconfig_checks[pkgconfig]
                if default_value is not None:
                    return default_value
                raise dependency_base.DependencyException(
                    f'Variable not found in {desired_dep_name}'
                )

            dep.get_variable = get_variable  # type: ignore[method-assign]
            dep.name = desired_dep_name

            if (self.dependencies.static_libraries.get(desired_dep_name) or
                    self.dependencies.rust_libraries.get(desired_dep_name)):  # fmt: skip
                dep.static = True
