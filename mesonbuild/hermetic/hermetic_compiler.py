...
class HermeticCompiler(Compiler):
    ...
    def _setup_compiler(self) -> Compiler:
        ...
        if not compiler_paths:
            raise MesonException(f'No compiler paths for language {self.language}')
        ...
        if compiler_paths.sdk_wrap:
            ...
            if cmd is not None:
                ...
                sysroot_rel = compiler_paths.sysroot_path or ''
...
class HermeticCLikeCompiler(HermeticCompiler):
    ...
    def compiles(self, code: T.Union[File, str], *,
        ...
        if not res[0] and isinstance(testname, str) and testname:
            if mode == CompileCheckMode.LINK:
                self.compiler_config['links']['fails'][testname] = True
            else:
                self.compiler_config['compiles']['fails'][testname] = True
        return res
