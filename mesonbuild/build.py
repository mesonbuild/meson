...
class Target:
    def __init__(self, name, subdir, subproject, build_by_default, for_machine: MachineChoice):
        if has_path_sep(name):
            ...
            mlog.warning('''Target "{0}" has a path separator in its name.
...
class BuildTarget(Target):
    ...
    def __init__(self, name, subdir, subproject, for_machine: MachineChoice, sources, objects, environment, kwargs):
        ...
        if not any([self.sources, self.generated, self.objects, self.link_whole]):
            raise InvalidArguments('Build target {0} has no sources.'.format(name))
    ...
    def check_unknown_kwargs_int(self, kwargs, known_kwargs):
        ...
        for k in kwargs:
            if k not in known_kwargs:
                unknowns.append(k)
        if len(unknowns) > 0:
            mlog.warning('Unknown keyword argument(s) in target {0}: (1).'.format(self.name,
                                                                                  ', '.join(unknowns)))
    ...
    def extract_objects(self, srclist):
        ...
        for src in srclist:
            ...
            if src not in self.sources:
                raise MesonException('Tried to extract unknown source {0}.'.format(src))
            obj_src.append(src)
...
This will become a hard error in a future Meson release.''')
        ...
        for i in extra_files:
            ...
            if not(os.path.isfile(trial)):
                raise InvalidArguments('Tried to add non-existing extra file {0}.'.format(i))
        ...
        for r in resources:
            ...
            if not os.path.isfile(trial):
                raise InvalidArguments('Tried to add non-existing resource {0}.'.format(r))
        ...
        if self.gnu_symbol_visibility != '':
            ...
            if self.gnu_symbol_visibility not in permitted:
                raise InvalidArguments('GNU symbol visibility arg {0} not one of: {1}',
    ...
    def add_pch(self, language, pchlist):
        if not pchlist:
            ...
            if not environment.is_header(pchlist[0]):
                raise InvalidArguments('PCH argument {0} is not a header.'.format(pchlist[0]))
        elif len(pchlist) == 2:
            if environment.is_header(pchlist[0]):
                ...
                raise InvalidArguments('PCH argument {0} is of unknown type.'.format(pchlist[0]))
        ...
        for f in pchlist:
            ...
            if not os.path.isfile(os.path.join(self.environment.source_dir, self.subdir, f)):
                raise MesonException('File {0} does not exist.'.format(f))
    ...
    def __init__(self, name, subdir, subproject, kwargs, absolute_paths=False, backend=None):
        ...
        for k in kwargs:
            if k not in CustomTarget.known_kwargs:
                unknowns.append(k)
        if len(unknowns) > 0:
            mlog.warning('Unknown keyword arguments in target {0}: {1}}'.format(self.name, ', '.join(unknowns)))
    ...
    def process_kwargs(self, kwargs, backend):
        ...
        for ed in extra_deps:
            ...
            if not isinstance(ed, (CustomTarget, BuildTarget)):
                raise InvalidArguments('Can only depend on toplevel targets: custom_target or build_target (executable or a library) got: {0}({1})'.format(type(ed), ed))
    ...
    def __init__(self, name, subdir, subproject, for_machine: MachineChoice, sources, objects, environment, kwargs):
        ...
        for s in self.sources:
            if not s.endswith('.java'):
                raise InvalidArguments('Jar source {0} is not a java file.'.format(s))
        for t in self.link_targets:
            if not isinstance(t, Jar):
                raise InvalidArguments('Link target {0} is not a jar target.'.format(t))
