...
class BuildTarget(Target):
    ...
    def __init__(self, name, subdir, subproject, for_machine: MachineChoice, sources, objects, environment, kwargs):
        ...
        self.d_features = {}
        self.pic = False
        self.pie = None  # type: T.Optional[bool]
        # Sources can be:
...
This will become a hard error in a future Meson release.''')
    ...
    def _extract_pic_pie(self, kwargs, arg: str) -> T.Optional[bool]:
        # Check if we have -fPIC, -fpic, -fPIE, or -fpie in cflags
        all_flags = self.extra_args['c'] + self.extra_args['cpp']
        if '-f' + arg.lower() in all_flags or '-f' + arg.upper() in all_flags:
            mlog.warning("Use the '{}' kwarg instead of passing '{}' manually to {!r}".format(arg, '-f' + arg, self.name))
            return True
        if '-fno-' + arg.lower() in all_flags or '-fno-' + arg.upper() in all_flags:
            mlog.warning("Use the '{}' kwarg instead of passing '{}' manually to {!r}".format(arg, '-fno-' + arg, self.name))
            return False
        ...
        if arg == 'pic':
            ...
            if not isinstance(val, bool):
                raise InvalidArguments(
                    'Argument {} to {!r} must be a boolean'.format(arg, self.name))
        else:
            ...
            if val not in {True, False, None}:
                raise InvalidArguments(
                    'Argument {} to {!r} must one of "true", "false", "default"'.format(arg, self.name))
        return val
