...
Try setting b_lundef to false instead.'''.format(self.coredata.options[OptionKey('b_sanitize')].value),
    ...
    def add_target(self, name: str, tobj, meson_target: bool = False) -> None:
        """Validate and add a target to coredata.

        :name: the name of the target
        :meson_target: This target is generated by Meson and can use the
            `meson-` prefix in it's name
        """
        if name == '':
            raise InterpreterException('Target name must not be empty.')
        if name.strip() == '':
            raise InterpreterException('Target name must not consist only of whitespace.')
        if not meson_target and name.startswith('meson-'):
