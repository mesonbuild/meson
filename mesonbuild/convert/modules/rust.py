...
from mesonbuild.interpreterbase import InterpreterException, TypedArgs
...
class ConvertRustModule(RustModule):
    ...
    def __init__(self, interpreter: ConvertInterpreter, project_config: ConvertProjectConfig,
    ...
    def bindgen(self, state: ModuleState, args: T.List, kwargs: FuncBindgen) -> ModuleReturnValue:
        ...
        rust_libs: T.List[ConvertId] = []
        for d in kwargs.get('dependencies', []):
            if d.found() and isinstance(d, dependency_base.ExternalDependency):
                found = self.project_config.lookup_external_dependency(d.name)
                if not found:
                    continue

                (dep, is_proc_macro) = found

                # TECHNICALLY, bindgen can depend on on C-libraries too, notably libclang. But this
                # is another instance of "submit a patch if you hit this case".
                rust_libs.append(dep)
