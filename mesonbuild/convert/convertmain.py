...
from mesonbuild.convert.precomputed.precomputed_platform import PlatformsToml, PlatformInfo
...
if T.TYPE_CHECKING:
    ...
    class ConvertOptions(SharedCMDOptions, Protocol):
        ...
        cmd_line_options: T.Dict[OptionKey, T.Any]


def generate(project_instance: ConvertProjectInstance, options: ConvertOptions,
             platform: PrecomputedPlatform, env: environment.Environment,
    ...
    This function configures the environment with the precomputed platform, runs the
    ...
    d: T.Dict[OptionKey, T.Union[str, bool]] = {
...
def create_default_options(args: ConvertOptions) -> ConvertOptions:
...
def convert_build_system(config_toml: ProjectConfigToml, platform_toml: PlatformsToml,
    ...
    For each configuration, it simulates a Meson build environment using a "precomputed" platform
    ...
    """
    dependencies = PrecomputedDependencies(dependencies_toml)
    platform_configs: T.Dict[str, PlatformInfo] = {
        tc['name']: tc for tc in platform_toml.get('platform', []) if 'name' in tc
    }
    ...
    for project_instance in project_config.get_project_instances():
        ...
        default_options.cmd_line_options = {
            OptionKey.from_string(k): v
            for k, v in project_instance.option_instance.meson_options.items()
        }
