...
from mesonbuild.convert.abstract.abstract_platform import AbstractPlatform
...
def generate(project_instance: ConvertProjectInstance, options: argparse.Namespace,
             platform: AbstractPlatform, env: environment.Environment,
    ...
    This function configures the environment with the abstract platform, runs the
    ...
    later emission.
    """
    env.machines.host = platform.platform_info.machine_info[MachineChoice.HOST]
    env.machines.build = platform.platform_info.machine_info[MachineChoice.BUILD]
    state_tracker.set_current_config(platform.platform_info, custom_select_instances)
    c_compiler = platform.create_c_compiler(MachineChoice.HOST)
    if c_compiler:
        env.coredata.compilers[MachineChoice.HOST]['c'] = c_compiler
    cpp_compiler = platform.create_cpp_compiler(MachineChoice.HOST)
    if cpp_compiler:
        env.coredata.compilers[MachineChoice.HOST]['cpp'] = cpp_compiler
    rust_compiler = platform.create_rust_compiler(MachineChoice.HOST)
    if rust_compiler:
        env.coredata.compilers[MachineChoice.HOST]['rust'] = rust_compiler
    c_compiler = platform.create_c_compiler(MachineChoice.BUILD)
    if c_compiler:
        env.coredata.compilers[MachineChoice.BUILD]['c'] = c_compiler
    cpp_compiler = platform.create_cpp_compiler(MachineChoice.BUILD)
    if cpp_compiler:
        env.coredata.compilers[MachineChoice.BUILD]['cpp'] = cpp_compiler
    rust_compiler = platform.create_rust_compiler(MachineChoice.BUILD)
    if rust_compiler:
    ...
    if hasattr(user_defined_options, 'cmd_line_options'):
        cmd_line_opts = {
            OptionKey.from_string(k): v for k, v in user_defined_options.cmd_line_options.items()
        }
    ...
    for target in build_info.get_build_targets().values():
        build_target: T.Optional[
            T.Union[ConvertInstanceStaticLibrary, ConvertInstanceSharedLibrary]
        ] = None
...
def choose_build_system(project_config: ConvertProjectConfig,
    ...
    if project_config.build_system == 'soong':
        state_tracker: ConvertStateTracker = ConvertStateTracker(project_config, SoongBackend())
...
def convert_build_system(config_toml: T.Dict[str, T.Any], platform_toml: T.Dict[str, T.Any],
                         dependencies_toml: T.Dict[str, T.Any],
                         options: argparse.Namespace) -> int:  # fmt: skip
    """
    ...
    For each configuration, it simulates a Meson build environment using an "abstract" platform
    ...
    platform_configs = {tc.get('name'): tc for tc in platform_toml.get('platform', [])}
    ...
    for project_instance in project_config.get_project_instances():
        ...
        with tempfile.TemporaryDirectory() as build_dir:
            ...
            env = environment.Environment(
                default_options.sourcedir, default_options.builddir, default_options
            )
            platform = AbstractPlatform(
                env,
                project_instance.host_platform,
                project_instance.build_platform,
                platform_configs,
                platform_toml,
            )
