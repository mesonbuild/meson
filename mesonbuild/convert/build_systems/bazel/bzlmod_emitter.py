...
def _emit_platforms(output_dir: str, state_tracker: ConvertStateTracker,
    ...
    for dep in sorted(list(backend.external_deps), key=lambda x: x.repo):
        extension_usage.append(
            EXTENSION_GENERAL_TAG_TEMPLATE.format(
                name=dep.repo, url=dep.source_url, sha256=dep.source_hash or ''
            )
        )
        repos.append(f'"{dep.repo}"')

    platforms_with_wrap = [tc for tc in state_tracker.all_platforms if tc.compilers_wrap]
    if not platforms_with_wrap:
        return (extension_usage, register_platforms, repos)
    ...
    for tc in platforms_with_wrap:
        ...
        extension_usage.append(
            EXTENSION_PLATFORM_TAG_TEMPLATE.format(
                ...
                build_file=f'//bazel/platforms:{name}_compiler.BUILD',
                file_type='zip',
            )
        )
        register_platforms.append(f'register_toolchains("//bazel/platforms:{name}_platform_cc")')
        repos.append(f'"{name}"')
        ...
        if tc.sysroot_wrap:
            ...
            extension_usage.append(
                EXTENSION_SYSROOT_TAG_TEMPLATE.format(
                    name=sysroot_name, url=sysroot_wrap.url, sha256=sysroot_wrap.sha256 or ''
                )
            )
        ...
        # Bzlmod Resolution:
        # We use stable names and resolve actual paths in the define_meson_platform macro.
        sysroot_repo = f'@{name}_sysroot' if tc.sysroot_wrap else ''
        sysroot_path = tc.sysroot_path or ''

        target_triple = f'{cpu}-{os_name}'

        with open(
            os.path.join(platforms_dir, f'{name}_compiler.BUILD'), 'w', encoding='utf-8'
        ) as f:
            f.write(
        ...
        platforms_build_content += PLATFORMS_BUILD_ENTRY_TEMPLATE.format(
            name=name, cpu=machine_info.cpu_family, os=machine_info.system
        )
...
def _emit_platform_extension(output_dir: str) -> None:
    bazel_dir = os.path.join(output_dir, 'bazel')
    os.makedirs(bazel_dir, exist_ok=True)
    shutil.copy(
        os.path.join(os.path.dirname(__file__), 'starlark', 'platforms.bzl'),
        os.path.join(bazel_dir, 'platforms.bzl'),
    )


def _emit_module_bazel(output_dir: str, state_tracker: ConvertStateTracker,
