...
# Why meson_cc_library / meson_cc_binary??
...
def _compile_and_link(ctx, output_type):
    ...
    flag_info = MesonFlagInfo(defines = all_defines, copts = all_copts, linkopts = all_linkopts)
    return linking_outputs, comp_context, linking_contexts, flag_info

def _meson_cc_library_impl(ctx):
    linking_outputs, comp_context, linking_contexts, flag_info = _compile_and_link(ctx, "dynamic_library")
...
def _meson_cc_binary_impl(ctx):
    linking_outputs, comp_context, _, flag_info = _compile_and_link(ctx, "executable")
...
meson_cc_library = rule(
    ...
    attrs = _meson_cc_attrs,
    fragments = ["cpp"],
    toolchains = ["@bazel_tools//tools/cpp:toolchain_type"],
)

meson_cc_binary = rule(
    implementation = _meson_cc_binary_impl,
    executable = True,
    attrs = _meson_cc_attrs,
    fragments = ["cpp"],
    toolchains = ["@bazel_tools//tools/cpp:toolchain_type"],
