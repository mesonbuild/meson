...
def _emit_target_block(soong_target_block: SoongTargetBlock) -> str:
    ...
    if soong_target_block.has(SoongTargetBlock.HOST_MULTILIB):
        content_str += TARGET_BLOCK_HOST
        content_str += MULTILIB_64
        content_str += 2 * COMMON_INDENT * ' ' + '},\n'

    if (soong_target_block.has(SoongTargetBlock.ANDROID_MULTILIB) or soong_target_block.has(SoongTargetBlock.ANDROID_INSTALL)):  # fmt: skip
        content_str += TARGET_BLOCK_ANDROID
        if (soong_target_block.has(SoongTargetBlock.ANDROID_MULTILIB) and not soong_target_block.has(SoongTargetBlock.ANDROID_32_BIT)):  # fmt: skip
            content_str += MULTILIB_64
        if soong_target_block.has(SoongTargetBlock.ANDROID_INSTALL):
