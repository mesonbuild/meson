...
from mesonbuild.convert.abstract.abstract_dependencies import AbstractDependencies
...
class ConvertProjectConfig:
    ...
    def __init__(self, config_data: T.Dict[str, T.Any],
    ...
    def project_path(self) -> str:
        return T.cast(str, self._toml_data.get('project', {}).get('project_path', ''))

    @property
    def handwritten_modules(self) -> T.Optional[str]:
        return T.cast(
            T.Optional[str], self._toml_data.get('project', {}).get('handwritten_modules')
        )
    ...
    def _get_option_combinations(self, static_options: T.Dict[str, T.Any],
