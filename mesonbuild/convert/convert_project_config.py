...
class ConvertProjectConfig:
    ...
    def lookup_external_dependency(self, dep_name: str) -> T.Optional[T.Tuple[ConvertId, bool]]:
        dep_info = (
            self.dependencies.shared_libraries.get(dep_name)
            or self.dependencies.static_libraries.get(dep_name)
            or self.dependencies.rust_libraries.get(dep_name)
            or self.dependencies.header_libraries.get(dep_name)
        )

        if not self.is_dependency_necessary(dep_name):
            return None
        ...
        is_proc_macro = bool(dep_info[0].get('proc_macro', False))
        dep = ConvertId(target_name, subdir, repo_name, source_url, source_filename, source_hash)

        return (dep, is_proc_macro)
