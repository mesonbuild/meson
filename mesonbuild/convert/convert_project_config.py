...
class ConvertProjectConfig:
    ...
    def handwritten_modules(self) -> T.Optional[str]:
        return T.cast(
            T.Optional[str],
            self._toml_data.get("project", {}).get("handwritten_modules"),
        )
    ...
    def _get_option_combinations(self,
                                 static_options: T.Dict[str, T.Any],
                                 variable_options: T.Dict[str, T.List[T.Any]]) -> T.List[ProjectOptionsInstance]:  # fmt: skip
        """
        Computes the Cartesian product of all variable Meson options.

        This function takes the `variable_options` defined in the TOML configuration and generates
        all possible unique combinations of them. Each combination results in a distinct
        `ProjectOptionsInstance`.

        A `ProjectInstance` is a fully resolved set of Meson options and the corresponding
        `SelectInstance`s that define that specific configuration variant. This allows the
        tool to analyze each permutation of the build configuration independently.
        """
        ...
        if not variable_options:
            ...
            for select in custom_selects:
                select_instances.append(
                    SelectInstance(select.select_id, select.default_value)
                )
    ...
    def get_project_instances(self) -> T.List[ConvertProjectInstance]:
        ...
        for config in configs:
            ...
            combinations = self._get_option_combinations(
                static_options, variable_options
            )
