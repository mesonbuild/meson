...
def determine_key(obj: object) -> T.Optional[str]:
...
class ConvertInterpreterInfo:
    ...
    def assign(self, name: str, subdir: str, obj: object) -> None:
        key = determine_key(obj)
        if key and key not in self.assignments:
            self.assignments[key] = (name, subdir)

    def lookup_assignment(self, obj: object) -> T.Optional[str]:
        key = determine_key(obj)
        if key and key in self.assignments:
            return self.assignments[key][0]

        return None

    def lookup_full_assignment(self, obj: object) -> T.Optional[T.Tuple[str, str]]:
        key = determine_key(obj)
        if key and key in self.assignments:
