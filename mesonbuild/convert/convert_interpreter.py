...
class ConvertInterpreter(HermeticInterpreter):
    ...
    def func_import(self, node: mparser.BaseNode, args: T.Tuple[str],
        ...
        if modname == 'rust':
            if 'rust' not in self.modules:
                self.modules['rust'] = ConvertRustModule(
                    self, self.project_config, self.project_instance
                )
    ...
    def track_assignment(self, name: str) -> None:
        holder = self.variables.get(name)
        if not holder:
            return

        variable: object = holder.held_object if isinstance(holder, ObjectHolder) else holder
