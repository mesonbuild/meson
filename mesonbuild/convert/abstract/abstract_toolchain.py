...
class AbstractToolchainInfo:
    ...
    def __init__(
        ...
        global_config: T.Optional[T.Dict[str, T.Any]] = None,
    ):
        ...
        if global_config:
            comp_wrap_config = global_config.get("compiler_binaries", {}).get("wrap")
            if comp_wrap_config:
                self.compilers_wrap = AbstractToolchainWrap(comp_wrap_config)
                # The compilers wrap has binaries at binary_paths in the TOML
                self.compilers_wrap.binaries = global_config.get(
                    "compiler_binaries", {}
                ).get("binary_paths", {})

            sysroot_wrap_config = global_config.get("sysroot", {}).get("wrap")
            if sysroot_wrap_config:
                self.sysroot_wrap = AbstractToolchainWrap(sysroot_wrap_config)
