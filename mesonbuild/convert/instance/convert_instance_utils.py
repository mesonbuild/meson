...
from mesonbuild.build import GeneratedList
...
class ConvertInstanceFlag:
    ...
    def __init__(self, name: str, subdir: str, language: str):
        ...
        self.rust_cfgs: T.List[str] = []
        self.link_args: T.List[str] = []

    def add_compile_arg(self, arg: str, rust_cfg: bool) -> None:
        arg = arg.replace('"', '\\"')
        if rust_cfg:
            self.rust_cfgs.append(arg)
            self.rust_cfgs.sort()
        else:
...
class ConvertInstanceRustBindgen(GeneratedList):
    def __init__(self, crate_name: str, subdir: str, header: ConvertId,
                 args: T.List[str], output: str,
                 rust_libs: T.List[ConvertId],
                 c_args: T.List[str]) -> None:  # fmt: skip
        self.subdir = subdir
        self.target_name: str = f'lib{crate_name}'
        self.crate_name = crate_name
        self.header = header
        self.args = args
        self.output = output
        self.rust_libs = rust_libs
        self.c_args = c_args
