...
def _determine_name(original_name: str, project_config: ConvertProjectConfig,
    ...
    if rust_abi is not RustABI.NONE:
...
class ConvertInstanceBuildTarget:
    ...
    def _handle_sources(self, build_target: build.BuildTarget,
        ...
        for file in build_target.sources:
            ...
            filegroup_or_include_dir = file.subdir != self.subdir or file.fname != os.path.basename(
                file.fname
            )
        ...
        if self.rust_abi is not RustABI.NONE:
            for s in self.srcs:
                if os.path.basename(s.name) == 'lib.rs':
                    self.crate_root = s.name
                    break
            if not self.crate_root and self.srcs:
                self.crate_root = self.srcs[0].name

            for s in self.srcs:
                self.src_subdirs.add(os.path.dirname(s.name))
            ...
            if d.found() and isinstance(d, dependency_base.ExternalDependency):
                ...
                if d.name in project_config.dependencies.header_libraries:
                    self.header_libs.append(dep)
                      ...
                      or d.name in project_config.dependencies.rust_libraries):  # fmt: skip
                    if is_proc_macro:
    ...
    def _handle_internal_dependencies(self, build_target: build.BuildTarget,
        ...
        for target in build_target.link_whole_targets:
            ...
            if self.rust_abi is RustABI.NONE and target_rust_abi is RustABI.RUST:
                continue
