...
class ConvertCustomTargetCmdPart:
    ...
    def from_convert_src(source: ConvertSrc) -> ConvertCustomTargetCmdPart:
        return ConvertCustomTargetCmdPart(
            "", ConvertCustomTargetCmdPartType.INPUT, source
        )
...
class ConvertInstanceCustomTarget:
    ...
    def __init__(
        ...
        self.python_depend_files: ConvertSrcList = ConvertSrcList()
    ...
    def get_python_target(self) -> T.Optional[ConvertInstancePythonTarget]:
        ...
        for tool in self.tools:
            if tool.target_only().endswith("_py_binary"):
                python_binary_name = tool.target_only()
                break

        if not python_binary_name:
            return None
        ...
        python_target = ConvertInstancePythonTarget(
            ...
            self.tool_subdir,
            python_binary_name,
            self.python_depend_files.get_sources(),
        )
    ...
    def _handle_file(self, file: File, custom_target: build.CustomTarget) -> None:
        needs_filegroup = file.subdir != self.subdir or file.fname != os.path.basename(
            file.fname
        )
        ...
        if is_python_script(file.fname) and self.python_script is None:
            self.python_script = ConvertSrc.from_target(
                os.path.basename(file.fname), subdir
            )
            python_binary = python_script_to_binary(file.fname)
            tool_src = ConvertSrc.from_target(python_binary, subdir)
            if tool_src in self.tools:
                return

            self.tool_subdir = subdir
            self.python_depend_files.add(src)
    ...
    def _handle_normalized_string(
        ...
        for part in string_parts:
            if part.startswith("-I"):
                ...
                depfile = custom_target.get_dep_outname(self.srcs[0].target_only())  # type: ignore
