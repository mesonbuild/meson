...
CustomTargetCommandElement = T.Union[
    programs.Program, str, File, build.BuildTarget, build.CustomTarget, build.CustomTargetIndex
]
...
class ConvertInstanceCustomTarget:
    ...
    def _parse_custom_target(self, custom_target: build.CustomTarget,
        ...
        for src in custom_target.sources:
            if isinstance(src, File) and is_python_script(src.fname) and self.python_tool:
        ...
        depend_files: T.Sequence[T.Union[File, str]] = custom_target.depend_files
        for file in depend_files:
            if isinstance(file, File):
                if is_python_script(file.fname) and self.has_python_cmd and self.python_tool:
