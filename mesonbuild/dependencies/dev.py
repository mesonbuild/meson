...
class ClangSystemDependency(SystemDependency):
    ...
    def __init__(self, name: str, env: Environment, kwargs: T.Dict[str, T.Any], language: T.Optional[str] = None) -> None:
        # Default to the C++ bindings, as they are more powerful and more
        # commonly used than the C bindings.
        ...
        for search in dirs:
            ...
            version = self.clib_compiler.get_define('CLANG_VERSION', '#include <clang/Basic/Version.inc>', env, libs, self.ext_deps)[0]
            if not self.version_reqs or (mesonlib.version_compare_many(version, self.version_reqs)):
                self.version = version
                self.link_args = libs
                self.is_found = True
