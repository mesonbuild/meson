...
class BLASLAPACKMixin(BLASLAPACKMixinBase, metaclass=abc.ABCMeta):
    @abc.abstractmethod
    ...
    def parse_modules(self, kwargs: DependencyObjectKWs) -> None:
        ...
        if interface:
            ...
            self.interface = interface[0].split(' ', 1)[1]
        else:
            self.interface = 'lp64'
    ...
    def check_symbols(self, compile_args: list[str], suffix: T.Optional[str] = None, check_cblas: bool = True,
                      check_lapacke: bool = True, lapack_only: bool = False) -> bool:
        ...
        return self.clib_compiler.links(code, extra_args=compile_args)[0]

    def get_variable(self, **kwargs: T.Any) -> str:
...
class OpenBLASMixin(BLASLAPACKMixin):
    ...
    def probe_symbols(self, compile_args: list[str]) -> bool:
...
class OpenBLASSystemDependency(OpenBLASMixin, SystemDependency):
    ...
    def detect(self, lib_dirs: T.Optional[T.List[str]] = None, inc_dirs: T.Optional[T.List[str]] = None) -> None:
        ...
        for libname in libnames:
            link_arg = self.clib_compiler.find_library(libname, lib_dirs)
            if link_arg:
                incdir_args = [f'-I{inc_dir}' for inc_dir in inc_dirs]
                for hdr in ['openblas_config.h', 'openblas/openblas_config.h']:
                    found_header, _ = self.clib_compiler.has_header(hdr, '', dependencies=[self],
...
class NetlibMixin(BLASLAPACKMixin):
    def get_symbol_suffix(self) -> str:
    ...
    def detect(self, lib_dirs: T.Optional[T.List[str]] = None, inc_dirs: T.Optional[T.List[str]] = None) -> None:
        ...
        for libname in libnames:
            ...
            if self.needs_cblas and not cblas_in_blas:
                ...
                link_arg_cblas = self.clib_compiler.find_library(cblas_libname, lib_dirs)
                if link_arg_cblas:
    ...
    def __init__(self, name: str, env: Environment, kwargs: DependencyObjectKWs) -> None:
        ...
        if self.is_found:
            if self.needs_lapacke:
                # Similar to CBLAS: there may be a separate lapacke
                # On Debian, it's lapack(64)?-netlib but lapacke(64)?
                lapacke_name = name.replace('-netlib', '').replace('lapack', 'lapacke')
                try:
                    lapacke_pc = PkgConfigDependency(lapacke_name, env, kwargs)
                    if lapacke_pc.found():
    ...
    def detect(self, lib_dirs: T.Optional[T.List[str]] = None, inc_dirs: T.Optional[T.List[str]] = None) -> None:
        ...
        for libname in libnames:
            ...
            if self.needs_lapacke and not lapacke_in_lapack:
                ...
                link_arg_lapacke = self.clib_compiler.find_library(lapacke_libname, lib_dirs)
                if link_arg_lapacke:
...
class MKLMixin(BLASLAPACKMixin):
    ...
    def parse_mkl_options(self, kwargs: DependencyObjectKWs) -> None:
        ...
        if not threading_modules:
            ...
            self.threading = opt.split(' ', 1)[1]
            if self.threading not in ('seq', 'iomp', 'gomp', 'tbb'):
        ...
        if not sdl_modules:
            self.use_sdl = 'auto'
        elif len(sdl_modules) > 1:
            raise mesonlib.MesonException(f'Multiple sdl arguments: {sdl_modules}')
        else:
            # We have a single sdl option specified - validate and process it
            opt = sdl_modules[0]
            self.use_sdl = opt.split(' ', 1)[1]
            if self.use_sdl not in ('true', 'false', 'auto'):
        ...
        if self.use_sdl == 'auto' and not self.sdl_default_opts:
            self.use_sdl = 'false'
        elif self.use_sdl != 'false' and not self.sdl_default_opts:
    ...
    def probe_version(self, compile_args: list[str]) -> bool:
        """Determine MKL version and ILP64 symbol suffix"""
        ver, _ = self.clib_compiler.get_define('INTEL_MKL_VERSION',
                                               '#include "mkl_version.h"',
                                               dependencies=[self],
                                               extra_args=compile_args)
        if len(ver) == 8:
            # mkl < 2025 uses (major * 10000 + minor * 100 + update)
            # mkl >= 2025 uses (major * 10000 + update * 100 + patch)
            # where releases are <major>.<update>.<patch>
            # and <minor> seems to always be 0
            major = int(ver[:-4])
            if major >= 2025:
                update = int(ver[-4:-2], 10)
                patch = int(ver[-2:], 10)
            else:
                update = int(ver[-2:], 10)
                patch = 0
            self.version = f'{major}.{update}.{patch}'
            self._ilp64_suffix = '_64' if (major, update) >= (2022, 2) else ''
            return True
        return False
...
class MKLPkgConfigDependency(MKLMixin, PkgConfigDependency):
    ...
    def __init__(self, name: str, env: Environment, kwargs: DependencyObjectKWs) -> None:
        ...
        if self.use_sdl == 'true':
            name = 'mkl-sdl'
        else:
            name = f'mkl-{libtype}-{self.interface}-{self.threading}'
        super().__init__(name, env, kwargs)

        if self.is_found and not self.probe_version(self.compile_args):
            self.is_found = False
...
class MKLSystemDependency(MKLMixin, SystemDependency):
    ...
    def __init__(self, name: str, environment: Environment, kwargs: DependencyObjectKWs) -> None:
        ...
        if self.use_sdl == 'true' and self.sdl_default_opts:
            self.detect_sdl()
    ...
    def detect_sdl(self) -> None:
        ...
        link_arg = self.clib_compiler.find_library('mkl_rt', lib_dirs)
        if link_arg:
            incdir_args = [f'-I{inc_dir}' for inc_dir in inc_dirs]
            found_header, _ = self.clib_compiler.has_header('mkl_version.h', '',
        ...
        if link_arg and found_header:
            ...
            if not self.probe_version(self.compile_args):
                self.is_found = False
