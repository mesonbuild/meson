...
class Interpreter(InterpreterBase, HoldableObject):
    ...
    def _do_subproject_cmake(self, subp_name: str, subdir: str,
        ...
        with mlog.nested(subp_name):
            prefix = self.coredata.optstore.get_value_for_safe(OptionKey('prefix'), str)
    ...
    def set_backend(self) -> None:
        ...
        if OptionKey('genvslite') in self.user_defined_options.cmd_line_options:
            ...
            backend_name = self.coredata.optstore.get_value_for_safe(OptionKey('genvslite'), str)
            self.backend = backends.get_genvslite_backend(backend_name, self.build, self)
        else:
            backend_name = self.coredata.optstore.get_value_for_safe(OptionKey('backend'), str)
    ...
    def func_project(self, node: mparser.FunctionNode, args: T.Tuple[str, T.List[str]], kwargs: 'kwtypes.Project') -> None:
        ...
        if not self.is_subproject():
            ...
            backend = self.coredata.optstore.get_value_for_safe(OptionKey('backend'), str)
            vsenv = self.coredata.optstore.get_value_for_safe(OptionKey('vsenv'), bool)
    ...
    def build_both_libraries(self, node: mparser.BaseNode, args: T.Tuple[str, SourcesVarargsType], kwargs: kwtypes.Library) -> build.BothLibraries:
        ...
        preferred_library = self.coredata.optstore.get_value_for_safe(OptionKey('default_both_libraries'), str)
        if preferred_library == 'auto':
            preferred_library = self.coredata.optstore.get_value_for_safe(OptionKey('default_library'), str)
    ...
    def build_library(self, node: mparser.BaseNode, args: T.Tuple[str, SourcesVarargsType], kwargs: kwtypes.Library):
        default_library = self.coredata.optstore.get_value_for_safe(OptionKey('default_library', subproject=self.subproject), str)
