...
class PythonInstallation(_ExternalProgramHolder['PythonExternalProgram']):
    def __init__(self, python: 'PythonExternalProgram', interpreter: 'Interpreter'):
        ...
        info = python.info
        prefix = self.interpreter.environment.coredata.get_option(OptionKey.factory('prefix'))
    ...
    def extension_module_method(self, args: T.Tuple[str, T.List[BuildTargetSource]], kwargs: ExtensionModuleKw) -> 'SharedModule':
        ...
        allow_limited_api = self.interpreter.environment.coredata.get_option(OptionKey.factory('python.allow_limited_api'))
...
class PythonModule(ExtensionModule):
    ...
    def _get_install_scripts(self) -> T.List[mesonlib.ExecutableSerialisation]:
        ...
        ret = []
        optlevel = self.interpreter.environment.coredata.get_option(OptionKey.factory('python.bytecompile'))
        if optlevel == -1:
            return ret
