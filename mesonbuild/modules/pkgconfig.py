...
class PkgConfigModule(NewExtensionModule):
    ...
    def _generate_pkgconfig_file(self, state: ModuleState, deps: DependenciesHelper,
        ...
        if uninstalled:
            ...
            prefix = PurePath(_as_str(coredata.get_option(OptionKey.factory('prefix'))))
            if pkgroot:
            ...
            for optname in optnames:
                if optname in referenced_vars - varnames:
                    if optname == 'prefix':
                        ...
                        dirpath = PurePath(_as_str(coredata.get_option(OptionKey.factory(optname))))
    ...
    def generate(self, state: ModuleState,
        ...
        if pkgroot is None:
            ...
            if m.is_freebsd():
                ...
                pkgroot = os.path.join(_as_str(state.environment.coredata.get_option(OptionKey.factory('prefix'))), 'develop', 'lib', 'pkgconfig')
                pkgroot_name = os.path.join('{prefix}', 'develop', 'lib', 'pkgconfig')
            else:
                pkgroot = os.path.join(_as_str(state.environment.coredata.get_option(OptionKey.factory('libdir'))), 'pkgconfig')
