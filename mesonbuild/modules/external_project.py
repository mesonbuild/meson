    ...
    def _configure(self, state: 'ModuleState') -> None:
        ...
        for lang, compiler in self.env.coredata.compilers[MachineChoice.HOST].items():
            if any(lang not in i for i in (ENV_VAR_PROG_MAP, CFLAGS_MAPPING)):
                continue
            cargs = self.env.coredata.optstore.get_value_for_untyped(
                OptionKey(f'{lang}_args', self.subproject, MachineChoice.HOST))
            ...
            if not link_exelist:
                ...
                _l = self.env.coredata.optstore.get_value_for_untyped(
                    OptionKey(f'{lang}_link_args', self.subproject, MachineChoice.HOST))
