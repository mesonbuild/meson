...
class GnomeModule(ExtensionModule):
    ...
    def compile_resources(self, state: 'ModuleState', args: T.Tuple[str, CustomTargetInputs],
        ...
        if isinstance(input_file, str):
            input_file = mesonlib.File.from_source_file(state.environment.get_source_dir(),
                                                        state.subdir, input_file)
        input_file = T.cast('CustomTargetSources', input_file)
    ...
    def gdbus_codegen(self, state: 'ModuleState', args: T.Tuple[str, T.Optional[CustomTargetInputs]],
                      kwargs: 'GdbusCodegen') -> ModuleReturnValue:
        namebase = args[0]
        xml_files: T.List[CustomTargetSources] = self.interpreter.source_strings_to_files([args[1]]) if args[1] else []
    ...
    def generate_vapi(self, state: 'ModuleState', args: T.Tuple[str], kwargs: 'GenerateVapi') -> ModuleReturnValue:
        ...
        for i in kwargs['sources']:
            if isinstance(i, str):
                ...
                FeatureNew.single_use('file object as source for gnome.generate_vapi', '1.12.0',
