...
class GnomeModule(ExtensionModule):
    ...
    def executable(self, state: 'ModuleState', args: T.Tuple[str], kwargs: kwtypes.Executable) -> ModuleReturnValue:
        if state.environment.machines.host.is_android():
            kwargs["gui_app"] = None
            kwargs["win_subsystem"] = None

            target = state.add_target(args, kwargs, build.SharedModule)
            glue = state.dependency('gtk4-android-glue')
            target.add_deps(glue)
        else:
            target = state.add_target(args, T.cast(kwtypes.SharedModule, kwargs), build.Executable)
