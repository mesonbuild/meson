...
if T.TYPE_CHECKING:
    ...
    class QmlModuleKwArgs(TypedDict):
        ...
        moc_sources: T.Sequence[T.Union[FileOrString, build.GeneratedTypes]]
        moc_headers: T.Sequence[T.Union[FileOrString, build.GeneratedTypes]]
        include_directories: T.List[T.Union[str, build.IncludeDirs]]
        imports: T.List[str]
        optional_imports: T.List[str]
        default_imports: T.List[str]
...
class QtBaseModule(ExtensionModule):
    ...
    def has_tools(self, state: 'ModuleState', args: T.Tuple, kwargs: 'HasToolKwArgs') -> bool:
        ...
        for tool in kwargs['tools']:
            assert tool in self.tools, f'tools must be in {self._set_of_qt_tools}'
        ...
        for s in sources:
            if isinstance(s, (build.CustomTarget, build.CustomTargetIndex)):
                for o in s.get_outputs():
                    content_files.append(File.from_built_file(
    ...
    def _gen_qrc(self, state: 'ModuleState', kwargs: 'GenQrcKwArgs') -> File:
        ...
        fileout_path = fileout.rel_to_builddir(state.environment.get_build_dir())
    ...
    def _gen_qmldir(self, state: 'ModuleState', kwargs: 'GenQmldirKwArgs') -> File:
        ...
        import_re = re.compile('^([a-zA-Z0-9]+(\\.[a-zA-Z0-9]+)*)(\\/((\\d+(\\.\\d+)*)|auto))?$')

        outfile = File.from_built_file(state.subdir, kwargs['output'])
        outfilepath = outfile.rel_to_builddir(state.environment.get_build_dir())
        if not os.path.isdir(state.environment.build_dir):
            os.mkdir(state.environment.build_dir)

        with open(outfilepath, 'w', encoding='utf-8') as fd:
