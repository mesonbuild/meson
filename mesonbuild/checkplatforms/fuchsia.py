...
from .defs import Platform, WrapInfo, ToolchainInfo, PlatformSysroot, compute_sha256, extract_zip
...
def generate_fuchsia_platforms(
        clang_instance_id: str,
        sdk_instance_id: str,
        run_compiler_checks_callback: T.Callable[
            [str, str, T.List[str], T.List[str]], Platform],
        clang_wrap_name: str = "fuchsia_clang",
        sdk_wrap_name: str = "fuchsia_sdk",
        toolchain_name: str = "fuchsia_clang_toolchain") -> T.List[Platform]:  # fmt: skip
    """
    Downloads a Fuchsia Clang platform and Core SDK to perform compiler checks.

    This function fetches both the platform and the SDK from Chrome Infrastructure Package
    Deployment (CIPD).
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for config in configurations:
            ...
            try:
                ...
                t.wraps[sdk_wrap_name] = WrapInfo(
                    name=sdk_wrap_name,
                    source_url=sdk_url,
                    source_filename='fuchsia-sdk-core-linux-amd64.zip',
                    source_hash=sdk_hash,
                )
                t.toolchains[toolchain_name] = ToolchainInfo(
                    name=toolchain_name, wrap_name=clang_wrap_name, binaries=wrap_binaries
                )
                t.toolchain = toolchain_name
                t.sysroot = PlatformSysroot(
                    wrap_name=sdk_wrap_name, path=f'arch/{config.sysroot_arch}/sysroot'
                )
                platforms.append(t)
            finally:
                if temp_cross_file:
