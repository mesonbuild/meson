...
''')
    ...
    def generate_target(self, target, outfile):
        ...
        vala_generated_headers = []
        if 'vala' in target.compilers:
            # Sources consumed by valac are filtered out. These only contain
            # C/C++ sources, objects, generated libs, and unknown sources now.
            target_sources, generated_sources, \
                vala_generated_sources, vala_generated_headers = self.generate_vala_compile(target, outfile)
        else:
        ...
        preexisting_header_deps = header_deps + [os.path.join('.', header) for header in vala_generated_headers]
        for f, src in target_sources.items():
            if not self.environment.is_header(src):
                if self.environment.is_llvm_ir(src):
                    ...
                    obj_list.append(self.generate_single_compile(target, outfile, src, False, [], preexisting_header_deps))
        obj_list += self.flatten_object_list(target)
        if is_unity:
            for src in self.generate_unity_files(target, unity_src):
                obj_list.append(self.generate_single_compile(target, outfile, src, True, unity_deps + preexisting_header_deps))
    ...
    def generate_vala_compile(self, target, outfile):
        ...
        vala_c_src = []
        # Header files generated by valac
        vala_headers = []
        ...
        element.write(outfile)
        return other_src[0], other_src[1], vala_c_src, vala_headers
