...
''')
    ...
    def generate_target(self, target):
        ...
        elif 'orc' in target.compilers:
            # Sources consumed by orccc are filtered out. These only contain
            # C/C++ sources, objects, generated libs, and unknown sources now.
            target_sources, generated_sources, \
                transpiled_sources = self.generate_orc_compile(target)
    ...
    def generate_orc_compile(self, target: build.BuildTarget) -> \
            T.Tuple[T.MutableMapping[str, File], T.MutableMapping[str, File], T.List[str]]:
        """Generate rules for transpiling Orc files to C"""

        orcc = target.compilers['orc']

        # C sources
        static_sources: T.MutableMapping[str, File] = OrderedDict()
        # C files generated by orcc [.c,.h]
        orc_c_sources: T.List[str] = []

        for src in target.get_sources():
            if src.endswith('.orc'):
                # Add the .c file
                output = os.path.join(self.get_target_private_dir(target), f'{src}.c')
                orc_c_sources.append(output)
        ...
        return static_sources, {}, orc_c_sources
    ...
    def generate_dynamic_link_rules(self):
        ...
        for for_machine in MachineChoice:
            ...
            for langname, compiler in complist.items():
                if langname in {'java', 'vala', 'orc', 'rust', 'cs', 'cython'}:
                    continue
    ...
    def generate_orc_compile_rules(self, compiler):
        rule = self.compiler_to_rule_name(compiler)
        command = compiler.get_exelist()
        args = NinjaCommandArg.list(compiler.get_output_args('$out'), Quoting.none)
        args += ['$ARGS']
        args += ['$in']
        description = 'Compiling Orc source $in to $out'
        self.add_rule(NinjaRule(rule, command + args, [], description, extra='restat = 1'))
...
https://gcc.gnu.org/bugzilla/show_bug.cgi?id=47485'''))
    ...
    def generate_compile_rule_for(self, langname, compiler):
        ...
        if langname == 'vala':
            ...
            return
        if langname == 'orc':
            self.generate_orc_compile_rules(compiler)
            return
        if langname == 'rust':
