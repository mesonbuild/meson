    ...
    def generate_target(self, target):
        ...
        if target.uses_fortran():
            fortran_inc_args = mesonlib.listify([target.compilers['fortran'].get_include_args(
                self.get_target_private_dir(t), IncludeType.NORMAL) for t in obj_targets])
    ...
    def generate_swift_target(self, target):
        ...
        for i in reversed(target.get_include_dirs()):
            ...
            for d in i.get_incdirs():
                ...
                sargs = swiftc.get_include_args(srctreedir, IncludeType.NORMAL)
                compile_args += sargs
        ...
        for x in abs_module_dirs:
            module_includes += swiftc.get_include_args(x, IncludeType.NORMAL)
...
https://gcc.gnu.org/bugzilla/show_bug.cgi?id=47485'''))
    ...
    def generate_inc_dir(self, compiler: 'Compiler', d: str, basedir: T.Optional[str], is_system: bool) -> \
        ...
        inctype = IncludeType.SYSTEM if is_system else IncludeType.NORMAL
        if basedir is None:
            return (compiler.get_include_args(d, inctype), [])
        ...
        sargs = compiler.get_include_args(srctreedir, inctype)
        ...
        if os.path.isdir(os.path.join(self.environment.get_build_dir(), expdir)):
            bargs = compiler.get_include_args(expdir, inctype)
        else:
            bargs = []
        return (sargs, bargs)
    ...
    def _generate_single_compile_target_args(self, target: build.BuildTarget, compiler: Compiler) -> ImmutableListProtocol[str]:
        ...
        for i in reversed(target.get_include_dirs()):
            ...
            for d in reversed(i.get_incdirs()):
                ...
                (compile_obj, includeargs) = self.generate_inc_dir(compiler, d, basedir, i.kind)
                commands += compile_obj
                commands += includeargs
            for d in i.get_extra_build_dirs():
                commands += compiler.get_include_args(d, i.kind)
        ...
        commands += compiler.get_include_args(self.get_target_private_dir(target), IncludeType.NORMAL)
        return commands
    ...
    def generate_msvc_pch_command(self, target, compiler, pch):
        ...
        if len(pch) == 1:
            ...
            commands += compiler.get_include_args(pch_header_dir, IncludeType.NORMAL)
        else:
