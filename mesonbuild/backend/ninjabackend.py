...
'''
    ...
    def scan_fortran_module_outputs(self, target):
        ...
        if compiler is None:
            ...
            return
        modre = re.compile(r"\bmodule\b\s+(\w+)\s*$", re.IGNORECASE)
        module_files = {}
        for s in target.get_sources():
    ...
    def get_fortran_deps(self, compiler: FortranCompiler, src: str, target) -> List[str]:
        ...
        submodre = re.compile(r"\bsubmodule\b\s+\((\w+)\)\s+(\w+)\s*$", re.IGNORECASE)
            ...
            for line in f:
                ...
                if usematch is not None:
                    ...
                    if submodmatch is not None:
                        parent = submodmatch.group(1).lower()
                        ...
                        if tdeps[parent].fname == os.path.basename(src):  # same file
                            continue
                        mod_name = compiler.module_name_to_filename(submodmatch.group(1))
                        mod_files.append(os.path.join(dirname, mod_name))
