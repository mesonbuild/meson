...
''')
    ...
    def generate_target(self, target, outfile: TextIO):
...
'''
    ...
    def scan_fortran_module_outputs(self, target):
        ...
        modre = re.compile(r"^(?<!sub)\s*module\s+(\w+)\s*$", re.IGNORECASE)
        # subrefre = re.compile(r"^(?<!sub)\s*module\s+(\w+)\s+(\w+).*$", re.IGNORECASE)
        # submodre = re.compile(r"^\s*submodule\s+\((\w+)\)\s+(\w+)\s*$", re.IGNORECASE)
...
# %% Fortran module dependency discovery
            with open(filename, errors='ignore') as f:
                for line in f:
                    ...
                    if modmatch is not None:
                        ...
                        continue
                    # else:
                    #    smodmatch = submodre.match(line)
                       # smodrefmatch = subrefre.match(line)
# %% Fortran submodule reference discovery
                    # NOTE: This is for the future, to resolve Fortran submodule between targets.
                    # if smodrefmatch is not None:
                    #    sref = smodrefmatch.group(2).lower()
                    #    if sref in smodref:
                    #        smodref[sref].append(s)
                    #    else:
                    #        smodref[sref] = [s]
                    ...
                    # else:
                    #    smodmatch = submodre.match(line)
# %% Fortran submodule dependency discovery

#                    if smodmatch is not None:
#                        smodname = smodmatch.group(2).lower()
#                        # pmodname = smodmatch.group(1).lower()
#                        if smodname in module_files:
#                            raise InvalidArguments(
#                                'Namespace collision: submodule %s defined in '
#                                'two files %s and %s.' %
#                                (smodname, module_files[smodname], s))

                        # target.is_unity = True
                        #  FIXME: in the future, resolve dependencies between targets.
                        # For now, have the user combine all submodule sources into
                        # one target.
                        # This is better than not Meson working at all with Fortran submodule.
