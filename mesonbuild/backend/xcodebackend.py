...
class XCodeBackend(backends.Backend):
    ...
    def __init__(self, build: T.Optional[build.Build], interpreter: T.Optional[Interpreter]):
        ...
        self.buildtype = T.cast('str', self.environment.coredata.get_option(OptionKey.factory('buildtype')))
    ...
    def get_target_dir(self, target: T.Union[build.Target, build.CustomTargetIndex]) -> str:
        dirname = os.path.join(target.get_subdir(), T.cast('str', self.environment.coredata.get_option(OptionKey.factory('buildtype'))))
    ...
    def generate_single_build_target(self, objects_dict, target_name, target) -> None:
        for buildtype in self.buildtypes:
            ...
            for lang in self.environment.coredata.compilers[target.for_machine]:
                ...
                warn_args = compiler.get_warn_args(target.get_option(OptionKey.factory('warning_level')))
            ...
            settings_dict.add_item('GCC_GENERATE_DEBUGGING_SYMBOLS', BOOL2XCODEBOOL[target.get_option(OptionKey.factory('debug'))])
            settings_dict.add_item('GCC_INLINES_ARE_PRIVATE_EXTERN', 'NO')
            opt_flag = OPT2XCODEOPT[target.get_option(OptionKey.factory('optimization'))]
