...
import functools, uuid, os, operator, re, subprocess
import typing as T
...
def autodetect_xcode_version() -> T.Tuple[str, int]:
    try:
        pc = subprocess.Popen(['xcodebuild', '-version'], encoding='utf-8', stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    version = int(re.search(r'\d*\.\d*\.*\d*', stdout).group(0).split('.'))
