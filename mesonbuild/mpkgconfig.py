...
def run(options: argparse.Namespace) -> int:
    ...
    if options.list_all:
        ...
        for pkg in packages:
            ...
            print(pkg.pkgname + align, pkg.get_name(), '-', pkg.get_description())
    ...
    for module in options.modules:
        ...
        if options.variable:
            print(pkg.get_variable(options.variable, overrides))
        if options.cflags:
            all_cflags += pkg.get_cflags(allow_system_cflags, overrides)
        if options.libs:
            all_libs += pkg.get_libs(options.static, allow_system_libs, overrides)
