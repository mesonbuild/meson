...
archive_choices = ['gztar', 'bztar', 'xztar', 'zip']
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--formats', default='xztar',
                        help='Comma separated list of archive types to create. Supports xztar (default), gztar, bztar, and zip.')
...
class HgDist(Dist):
    ...
    def create_dist(self, archives: T.List[str]) -> T.List[str]:
        ...
        if 'bztar' in archives:
            import bz2
            with bz2.open(bzname, 'wb') as bf, open(tarname, 'rb') as tf:
