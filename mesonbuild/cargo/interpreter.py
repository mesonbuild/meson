...
class PackageState:
    ...
    def supported_abis(self) -> T.Set[RUST_ABI]:
        ...
        if any(ct in {'lib', 'rlib'} for ct in crate_types):
            abis.add('rust')
...
class Interpreter:
    ...
    def resolve_package(self, package_name: str, api: T.List[str]) -> T.Optional[PackageState]:
    ...
    def _prepare_dependency(self, pkg: PackageState, dep: Dependency, dep_pkg: PackageState) -> None:
    ...
    def _add_dependency(self, pkg: PackageState, depname: str) -> None:
        cfg = pkg.cfg
        if depname in cfg.visited_deps:
            return
        cfg.visited_deps.add(depname)
        ...
        if pkg.dev_dependencies:
            ...
            if dep:
                ...
                self._prepare_dependency(pkg, dep, dep_pkg)
        dep = pkg.manifest.dependencies.get(depname)
        if dep:
            dep_pkg = dep_pkg or self._dep_package(pkg, dep)
            self._prepare_dependency(pkg, dep, dep_pkg)
        if dep_pkg is not None:
            pkg.cfg.required_deps.add(depname)
            for f in pkg.cfg.optional_deps_features[depname]:
                self._enable_feature(dep_pkg, f)
    ...
    def _create_project(self, name: str, pkg: T.Optional[PackageState], build: builder.Builder) -> T.List[mparser.BaseNode]:
        ...
        return [
            ...
            build.assign(build.method('workspace', build.identifier('rust')), 'cargo')
        ]
    ...
    def _create_dependency(self, pkg: PackageState, dep: Dependency, build: builder.Builder) -> T.List[mparser.BaseNode]:
        ...
        if self.cargolock and self.resolve_package(dep.package, api):
            dep_obj = build.method(
                'dependency',
                build.method(
                    'subproject',
                    build.identifier('cargo'),
                    [build.string(dep.package), build.string(api)]))
        else:
