...
class CythonCompiler(Compiler):
    ...
    def _transpiled_sanity_check_compile_args(self, compiler: Compiler, sourcename: str, binname: str) -> T.List[str]:
        args = super()._transpiled_sanity_check_compile_args(compiler, sourcename, binname)
        ...
        with mlog.no_logging():
            dep = find_external_dependency(f'python{version}', self.environment, {'required': False})
        if not dep.found():
    ...
    def _sanity_check_compile_args(self, sourcename: str, binname: str) -> T.List[str]:
        return super()._sanity_check_compile_args(sourcename, binname) + \
            self.get_option_compile_args(None)
