...
class ValaCompiler(Compiler):
    ...
    def sanity_check(self, work_dir, environment):
        ...
        ctx = self.cached_compile(code, environment.coredata, extra_args=extra_flags, mode='compile')
        with ctx.wait() as p:
            if p.returncode != 0:
    ...
    def find_library(self, libname, env, extra_dirs, *args):
        ...
        if not extra_dirs:
            ...
            args += vapi_args
            ctx = self.cached_compile(code, env.coredata, extra_args=args, mode='compile')
            with ctx.wait() as p:
                if p.returncode == 0:
                    return vapi_args
