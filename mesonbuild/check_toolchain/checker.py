...
def run_compiler_checks(
        ...
        name: str,
        c_flags: T.List[str],
        cpp_flags: T.List[str]) -> Toolchain:  # fmt: skip
    """
    ...
    Any failures or unsupported features are recorded in a `Toolchain` object,
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if cc.get_id() == "gcc" and mesonlib.version_compare(cc.version, "< 4.4.6"):
            raise mesonlib.MesonException(
                "When using GCC, version 4.4.6 or later is required."
            )
        ...
        if not cc.compiles("__uint128_t foo(void) { return 0; }", extra_args=c_flags)[
            0
        ]:
        ...
        if not cc.has_header_symbol("sys/sysmacros.h", "major", "", extra_args=c_flags)[
            0
        ]:
            results.c_header_symbols_fails.setdefault("sys/sysmacros.h", []).append(
                "major"
            )

        if not cc.has_header_symbol("sys/sysmacros.h", "minor", "", extra_args=c_flags)[
            0
        ]:
            results.c_header_symbols_fails.setdefault("sys/sysmacros.h", []).append(
                "minor"
        ...
        if not cc.has_header_symbol(
            "sys/sysmacros.h", "makedev", "", extra_args=c_flags
        )[0]:
            results.c_header_symbols_fails.setdefault("sys/sysmacros.h", []).append(
                "makedev"
            )
        ...
        if not cc.has_header_symbol("sys/mkdev.h", "makedev", "", extra_args=c_flags)[
            0
        ]:
            results.c_header_symbols_fails.setdefault("sys/mkdev.h", []).append(
                "makedev"
            )
        ...
        if not cc.links(
            "int main() { return 0; }", extra_args=["-Wl,-Bsymbolic"] + c_flags
        )[0]:
...
int main() {
  ...
  return 0;
        ...
        if not cpp.links(gnu_qsort_r, extra_args=cpp_flags)[0]:
        ...
        if not cpp.links(bsd_qsort_r, extra_args=cpp_flags)[0]:
            results.cpp_links_fails.add("qsort_r")
