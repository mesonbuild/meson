...
if T.TYPE_CHECKING:
    ...
    from mesonbuild.convert.precomputed.precomputed_platform import PlatformsToml
    from mesonbuild.convert.precomputed.precomputed_dependencies import DependenciesToml
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--platform', help='The path to a valid platform config file (toml).')
...
def run(options: ConvertOptions) -> int:
    ...
    if options.hermetic_project and options.git_project:
        base_path = os.path.join(
            os.path.dirname(__file__), 'convert', 'reference', options.hermetic_project
        )
        options.config = os.path.join(base_path, f'{options.git_project}.toml')
        options.platform = os.path.join(base_path, 'platforms.toml')
    ...
    try:
        ...
        with open(options.platform, 'rb') as f:
