...
def guess_nix_linker(env: 'Environment', compiler: T.List[str], comp_class: T.Type['Compiler'],
    ...
    if env.machines[for_machine].is_os2() and env.coredata.get_option(OptionKey('emxomf')):
        check_args += ['-Zomf']
    ...
    # Note that "ld: unknown option: " sometimes instead is "ld: unknown options:".
        ...
        linker = linkers.OS2AoutDynamicLinker(compiler, for_machine, comp_class.LINKER_PREFIX, override, version='none')
    elif 'emxomfld: invalid option' in e:
        linker = linkers.OS2OmfDynamicLinker(compiler, for_machine, comp_class.LINKER_PREFIX, override, version='none')
    else:
        __failed_to_detect_linker(compiler, check_args, o, e)
    return linker
